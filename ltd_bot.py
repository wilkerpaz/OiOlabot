...
async def send_daily_liturgy_audio(chat_id, path_audio, date):
    try:
        ...
        chat_username = chat.username if (chat.username and chat.type != 'private') else None
        caption = date + '\n\nt.me/' + (chat_username or BOT_NAME)
        send = await bot.send_audio(chat_id, audio=path_audio, caption=caption, file_name=f'{date}.mp3', title=date)
        return send
    except RPCError as _:
        errors(chat_id)
...
if __name__ == "__main__":
    try:
        ...
        start_date = DateHandler.combine(last_date.date(), DateHandler.time('07:00:00-03:00'))
