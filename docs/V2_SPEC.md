...
> - O plano da §7 de um `FeedJob` parametrizado servindo os dois bancos ficou incompleto por um tempo (`LiturgyDatabase` não tinha os métodos de assinatura de URL que `FeedMixin` chama, quebrando `/addurl`/`/listurl`/`/removeurl` no bot de liturgia) — **corrigido em 2026-09-13**: `LiturgyDatabase` ganhou os mesmos métodos de `MainDatabase`, e `worker.py` agora roda um segundo `FeedJob` dedicado ao DB 1, como sempre foi o plano original.
