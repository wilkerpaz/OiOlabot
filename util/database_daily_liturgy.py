...
class DatabaseHandler(object):
    ...
    '''extract chat_id for name'''
    @staticmethod
    def extract_chat_id_from_names(names):
        if names:
            uncompress_name = [name.split(':') for name in names]
            chat_id = sorted(set(['{}'.format(chat_id[-1]) for chat_id in uncompress_name]))
            return chat_id
        return ()
    ...
    '''activated all url'''
    def activated_all_chat_id(self):
        names = self._find('daily_liturgy*')
        for name in names:
    ...
    '''get chat_id for user_id and chat_name'''
